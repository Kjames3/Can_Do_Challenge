...
TARGET_IMAGES = 600
OUTPUT_DIR = "dataset/golden_test_set"
...
async def gather_golden_set():
    ...
    try:
        async with websockets.connect(SERVER_URI) as websocket:
            print(f"Connected! Beginning active sampling for {TARGET_IMAGES} frames...")
            
            while collected_count < TARGET_IMAGES:
                message = await websocket.recv()
                data = json.loads(message)
                
                # We only care about telemetry readouts that include camera frames
                if data.get("type") == "readout" and data.get("image"):
                    ...
                    # IMU yaw rate is typically in deg/s or rad/s, we'll take absolute value
                    angular_velocity_mag = abs(imu_data.get("yaw_rate", 0.0))
                    
                    # 2. Check movement threshold (Must actively be moving for motion blur)
                    ...
                    if is_moving:
                        ...
                        if img_bgr is not None:
                            # 4. Check blur severity (Variance of Laplacian)
                            ...
                            if blur_metric < MAX_LAPLACIAN_VARIANCE:
                                ...
                                # Save the mathematical / kinematic metadata
                                metadata_record = {
                                    "timestamp": timestamp,
                                ...
                                collected_count += 1
                                print(f"[{collected_count}/{TARGET_IMAGES}] Saved blurred frame (Metric: {blur_metric:.2f}, LinVel: {linear_velocity_mag:.2f}, AngVel: {angular_velocity_mag:.2f})")
